...
class FormTypeAdminmenuMenuelement extends AbstractForm
{
	public const string ID = 'admin_menu_item';
	...
	public function commit(): void
	{
		...
		switch ($this->getMode()) {
			case self::_MODE_CREATE:
				...
				$new_id = AdminMenu::addMenu($this->savedata, $parent_id);
			...
			case self::_MODE_UPDATE:
				...
				$changed = AdminMenu::updateMenu($this->savedata, $this->getItemId());
	...
	public function setMetadata(): void
	{
		if ($this->_mode == self::_MODE_CREATE) {
			$this->_meta->title = t('cms.admin_menu.form.title_create');
		} else {
			$this->_meta->title = t('cms.admin_menu.form.title_edit');
			$this->_meta->sub_title = AdminMenu::getMenuName($this->getItemId());
		}
	}

	public function setInitValues(): void
	{
		$this->initvalues = AdminMenu::getMenuValues($this->getItemId());
