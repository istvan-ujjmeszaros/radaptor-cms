...
class EventJstreeResourceAclUsergroupSelectorAjaxRemoveUsergroup extends AbstractEvent
{
	...
	public function run(): void
	{
		...
		$success = ResourceTreeHandler::removeFromUsergroup($usergroup_id, $resource_id);
		$target = $resource_data['path'] . $resource_data['resource_name'];

		if ($success) {
			SystemMessages::_ok(t('cms.resource_acl.usergroup_assignment.removed', ['group' => $usergroup_data['title'], 'target' => $target]));
		} else {
			SystemMessages::_error(t('cms.resource_acl.usergroup_assignment.error_remove', ['group' => $usergroup_data['title'], 'target' => $target]));
		}
		...
		if ($json_data['success']) {
			...
			ApiResponse::renderErrorObj(new ApiError('OPERATION_FAILED', t('cms.resource_acl.usergroup_assignment.error_remove_generic')), 400);
		}
	}
}
