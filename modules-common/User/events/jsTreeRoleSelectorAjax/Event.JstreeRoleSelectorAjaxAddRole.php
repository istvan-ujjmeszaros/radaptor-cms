...
class EventJstreeRoleSelectorAjaxAddRole extends AbstractEvent
{
	...
	public function run(): void
	{
		...
		switch ($for_type) {
			case 'user':
				...
				$success = Roles::assignToUser($role_id, $for_id);

				if ($success) {
					SystemMessages::_ok(t('user.role_assignment.added', ['role' => $role_data['title'], 'target' => $user_data['username']]));
				} else {
					SystemMessages::_error(t('user.role_assignment.error_add', ['role' => $role_data['title'], 'target' => $user_data['username']]));
				}
			...
			case 'usergroup':
				...
				$success = Roles::assignToUsergroup($role_id, $for_id);

				if ($success) {
					SystemMessages::_ok(t('user.role_assignment.added', ['role' => $role_data['title'], 'target' => $usergroup_data['title']]));
				} else {
					SystemMessages::_error(t('user.role_assignment.error_add', ['role' => $role_data['title'], 'target' => $usergroup_data['title']]));
				}
		...
		if ($success) {
			...
			ApiResponse::renderErrorObj(
				new ApiError('OPERATION_FAILED', t('user.role_assignment.error_add_generic')),
				400
			);
		}
	}
