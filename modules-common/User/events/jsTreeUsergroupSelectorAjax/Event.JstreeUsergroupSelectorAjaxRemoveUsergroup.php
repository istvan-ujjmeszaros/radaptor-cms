...
class EventJstreeUsergroupSelectorAjaxRemoveUsergroup extends AbstractEvent
{
	...
	public function run(): void
	{
		...
		$success = Usergroups::removeFromUser($usergroup_id, $user_id);

		if ($success) {
			SystemMessages::_ok(t('user.usergroup_assignment.removed', ['group' => $usergroup_data['title'], 'user' => $user_data['username']]));
		} else {
			SystemMessages::_error(t('user.usergroup_assignment.error_remove', ['group' => $usergroup_data['title'], 'user' => $user_data['username']]));
		}
		...
		if ($success) {
			...
			ApiResponse::renderErrorObj(
				new ApiError('OPERATION_FAILED', t('user.usergroup_assignment.error_remove_generic')),
				400
			);
		}
	}
